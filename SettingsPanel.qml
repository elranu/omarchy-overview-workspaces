...
Panel {
    ...
    // Every setting below is a labelled row with an ON/OFF pill, so the shape
    // lives here once instead of being spelled out for each one.
                ...
                Flickable {
                    ...
                    Column {
                        ...
                        ToggleRow {
                            ...
                            title: "Occupied workspaces only"
                            detail: GlobalStates.overviewSortMode === "legacy"
                                ? "Only workspaces with windows, in MRU order. Win+number follows these slots."
                                : "Native slots 1–10 stay visible and Win+number goes to the native workspace. Occupied workspaces still follow MRU order."
                            checked: GlobalStates.overviewSortMode === "legacy"
                            onToggled: root.persistMode(GlobalStates.overviewSortMode === "legacy" ? "system" : "legacy")
                        }
